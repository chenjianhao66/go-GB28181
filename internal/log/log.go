...
func getLoggerWrite() zapcore.WriteSyncer {
	...
	// 定义日志切割配置
	l := &lumberjack.Logger{
		Filename:   path.Join(config.LogFilePath(), config.LogFileName()), //Filename 是要写入日志的文件。
		MaxSize:    config.LogMaxSize(),                                   //MaxSize 是日志文件在轮换之前的最大大小（以兆字节为单位）。它默认为 100 兆字节
		MaxBackups: config.LogMaxBackups(),                                //MaxBackups 是要保留的最大旧日志文件数。默认是保留所有旧的日志文件（尽管 MaxAge 可能仍会导致它们被删除。）
		MaxAge:     config.LogMaxAge(),                                    //MaxAge 是根据文件名中编码的时间戳保留旧日志文件的最大天数。
		Compress:   true,                                                  //压缩
